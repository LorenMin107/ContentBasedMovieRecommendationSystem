...
// Getting individual cast details
...
function get_recommendations(movie_id, my_api_key) {
    ...
    $.ajax({
        ...
        success: function(recs) {
            let top_recs = recs.results.length >= 10 ? Array.from({ length: 10 }, (_, i) => i) : Array.from({ length: recs.results.length }, (_, i) => i);
            for (let i of top_recs) {
                rec_ids.push(recs.results[i].id);
                rec_movies.push(recs.results[i].title);
                rec_movies_org.push(recs.results[i].original_title);
                rec_year.push(recs.results[i].release_date.split('-')[0]);
                rec_vote.push(recs.results[i].vote_average);
                rec_posters.push(recs.results[i].poster_path ? 'https://image.tmdb.org/t/p/original' + recs.results[i].poster_path : 'static/default.jpg');
            }
        },
        error: function(error) {
            alert("API Error! - " + error);
