$(function() {
  ...
  $('.movie-button').on('click', function() {
    var my_api_key = 'a01db429ba34675b395e935cae21a21e';
    var title = $('.movie').val();
    if (title == "") {
  ...
  $.ajax({
    ...
    success: function(recs) {
      if (recs == "Sorry! The movie you requested is not in our database. Please check the spelling or try with some other movies") {
        ...
        if (movie_arr.length < 6) {
          movie_arr = [...movie_arr, ...Array(6 - movie_arr.length).fill("Placeholder Movie")];
        }
...
function get_movie_cast(movie_id, my_api_key) {
  ...
  $.ajax({
    ...
    success: function(my_movie) {
      var top_cast = my_movie.cast.length >= 10 ? top_10 : my_movie.cast.map((_, i) => i);
    ...
    $.ajax({
      ...
      success: function(m_data) {
        if (m_data.results.length > 0 && m_data.results[0].poster_path) {
          ...
          arr_poster_list.push(''); // Use a placeholder or empty string if no poster is found
        }
      },
      error: function() {
        alert("Invalid Request!");
